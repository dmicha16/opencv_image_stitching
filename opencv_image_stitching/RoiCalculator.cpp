		...
		for (size_t j = 0; j < num_rect_; j++) {
			if (i == 1) {
				for (size_t k = 0; k < matched_keypoints_.image_1.size(); k++) {
					if (rect_s_.rectangles[j].contains(matched_keypoints_.image_1[k])) {
...
int RoiCalculator::num_occupied_rects() {
	...
	for (size_t i = 0; i < rect_s_.rectangles.size(); i++) {
		for (size_t j = 0; j < matched_keypoints_.image_1.size(); j++) {
			...
			if (rect_s_.rectangles[i].contains(coord_to_check)) {
				num_occupied_rects++;
				break;
			}
		}
...
void Rectengales::populate_rectengales(int height_offset, int desired_cols) {
